...
- **My TIDAL**: your favorite albums, tracks, artists, videos, playlists, and mixes, with virtualized infinite scroll so large libraries stay smooth. It opens on a **Home** tab that previews your newest additions, up to 24 recent albums and 18 recent tracks; click any shelf heading to open the full list in that tab. Sort any category by recently added, name, release date, or artist, the same control as Search. Every category also has a **Download all** button that counts what it found and asks before queuing it: all your favorite tracks, albums or videos, every favorite artist's full discography, or every playlist (folders included) and mix. Your TIDAL playlist folders appear under Playlists and drill in like a file manager; each folder offers "Download all" with a count badge that ticks down like an odometer as playlists finish, and on disk your playlists mirror the same folder tree (customizable via a `{folder_path}` token in the path template). Every playlist page also offers **Download full albums** beside Download playlist: it saves the complete album each song comes from, one copy per album, following the same edition rules as a discography.
